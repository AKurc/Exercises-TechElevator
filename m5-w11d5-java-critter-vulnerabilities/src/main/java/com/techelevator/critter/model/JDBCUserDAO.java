...
public class JDBCUserDAO implements UserDAO {
	...
	public boolean searchForUsernameAndPassword(String userName, String password) {
		...
		if(results.next()) {
			String dbSalt = results.getNString("salt");
			String dbHashedPassword = results.getString("password");
			String passwordCheck = passwordHasher.computeHash(password, Base64.decode(dbSalt)); //base64 is a way to turn a byte array into a string and vice versa
			return passwordCheck.equals(dbHashedPassword);
		}
		return false; //if no results in db, this returns false
	}
